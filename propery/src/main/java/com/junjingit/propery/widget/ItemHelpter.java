 ...
 */
public class ItemHelpter implements GestureDetector.OnGestureListener,
        RecyclerView.OnItemTouchListener
{
            ...
            float velocityY)
    {
        if (Math.abs(velocityX) > mMinVelocity
                && Math.abs(velocityX) < mMaxVelocity)
        {
            if (!smoothView(false))
            {
                if (isClosed())
    ...
    {
        return mSwipeLayout != null
                && mSwipeLayout.mCenterView.getScrollX() == mSwipeLayout.mRightWidth;
    }
    ...
    {
        return mSwipeLayout != null
                && mSwipeLayout.mCenterView.getScrollX() == 0;
    }
    ...
    private boolean smoothView(Boolean isClose)
    {
        ...
        mAnimator = ObjectAnimator.ofInt(mSwipeLayout.mCenterView,
                "scrollX",
                to);
