 ...
 * the License.
 */
public class SpringFloatingActionMenu extends FrameLayout implements
        ViewTreeObserver.OnGlobalLayoutListener
{
    ...
    private void init(Context context)
    {
        ...
        //add FAB
        MarginLayoutParams fablp = Utils.createWrapParams(getResources());
            ...
            int bottom)
    {
        ...
        else
        {
            throw new IllegalStateException(
                    "gravity only support bottom center and bottom right");
        }
        ...
        for (ImageButton circle : mFollowCircles)
        {
            ...
            circle.layout(x,
                    y,
                    x + circle.getMeasuredWidth(),
                    y + circle.getMeasuredHeight());
        }
        
        int x = fabCenterX - mRevealCircle.getWidth() / 2;
        int y = fabCenterY - mRevealCircle.getHeight() / 2;
        mRevealCircle.layout(x, y, x + mRevealCircle.getMeasuredWidth(), y
                + mRevealCircle.getMeasuredHeight());
    ...
    private void layoutMenuItemsAsGrid(int left, int top, int right, int bottom)
    {
        ...
        int colCount = 3;
        int rowCount = mMenuItemCount % colCount == 0 ? mMenuItemCount
                / colCount : mMenuItemCount / colCount + 1;
        ...
        int itemGap = (containerWidth - 2 * edgeGap - colCount * itemWidth)
                / (colCount - 1);
        //top and bottom gap
        int tbGap = (containerHeight - rowCount * itemHeight - itemGap
                * (rowCount - 1)) / 2;
        ...
        for (int rowIdx = 0; rowIdx < rowCount; rowIdx++)
        {
            for (int colIdx = 0; colIdx < colCount; colIdx++)
            {
                ...
                if (idxInItem < mMenuItemCount)
                {
                    ...
                    Log.d(TAG, "menu item index:"
                            + (rowIdx * colCount + colIdx));
                    int itemLeft = edgeGap + colIdx * itemWidth + colIdx
                            * itemGap;
                    int itemTop = tbGap + rowIdx * itemGap + rowIdx
                            * itemHeight;
                    item.layout(itemLeft,
                            itemTop,
                            itemLeft + itemWidth,
                            itemTop + itemHeight);
                }
    ...
    private void layoutMenuItemsAsTumblr(int left, int top, int right,
            int bottom)
    {
        if (mMenuItemCount < 4 || mMenuItemCount > 7)
            throw new IllegalStateException(
                    "layout as tumblr style must have morn than 4 items,less than 7 items");
        int itemHeight = mMenuItemViews.get(0).getMeasuredHeight();
        int itemWidth = mMenuItemViews.get(0).getMeasuredWidth();
        ...
        firstItem.layout(firstX, firstY, firstX + itemWidth, firstY
                + itemHeight);
        ...
        for (int i = 0; i < mMenuItemCount - 1; i++)
        {
            ...
            item.layout(x,
                    y,
                    x + item.getMeasuredWidth(),
                    y + item.getMeasuredHeight());
        }
    ...
    private ArrayList<ImageButton> generateFollowCircles()
    {
        ...
        for (MenuItem item : mMenuItems)
        {
            ...
            shapeDrawable.getPaint()
                    .setColor(getResources().getColor(item.getBgColor()));
    ...
    private ArrayList<MenuItemView> generateMenuItemViews()
    {
        ...
        for (MenuItem item : mMenuItems)
        {
            ...
            menuItemView.setLayoutParams(Utils.createWrapParams(getResources()));
    ...
    private View generateRevealCircle()
    {
        ...
        OvalShape ovalShape = new OvalShape();
        ShapeDrawable shapeDrawable = new ShapeDrawable(ovalShape);
        shapeDrawable.getPaint().setColor(ContextCompat.getColor(mContext,
                mRevealColor));
    ...
    private void applyFollowAnimation()
    {
        ...
        new Actor.Builder(springSystem, mFAB).addMotion(springX,
                Imitator.TRACK_DELTA,
                Imitator.FOLLOW_EXACT,
                MotionProperty.X)
                .addMotion(springY,
                        Imitator.TRACK_DELTA,
                        Imitator.FOLLOW_EXACT,
        ...
        for (int i = 0; i < mFollowCircles.size(); i++)
        {
            ...
            followsX[i].addListener(new Performer(mFollowCircles.get(i),
                    View.TRANSLATION_X));
            followsY[i].addListener(new Performer(mFollowCircles.get(i),
                    View.TRANSLATION_Y));
    ...
    private void applyBloomOpenAnimation()
    {
        ...
        for (int i = 0; i < mMenuItemCount; i++)
        {
            ...
            springX.addListener(new MapPerformer(menuItemView, View.X,
                    mFAB.getLeft(), menuItemView.getLeft()));
            springY.addListener(new MapPerformer(menuItemView, View.Y,
                    mFAB.getTop(), menuItemView.getTop()));
            DestroySelfSpringListener destroySelfSpringListener = new DestroySelfSpringListener(
                    this, mContainerView, true);
    ...
    private void applyBloomCloseAnimation()
    {
        ...
        for (int i = 0; i < mMenuItemCount; i++)
        {
            ...
            MenuItemView menuItemView = mMenuItemViews.get(i);
            springX.addListener(new MapPerformer(menuItemView, View.X,
                    menuItemView.getLeft(), mFAB.getLeft()));
            springY.addListener(new MapPerformer(menuItemView, View.Y,
                    menuItemView.getTop(), mFAB.getTop()));
            DestroySelfSpringListener destroySelfSpringListener = new DestroySelfSpringListener(
                    this, mContainerView, false);
            springX.addListener(destroySelfSpringListener);
            springY.addListener(destroySelfSpringListener);
            springX.setEndValue(1);
            springY.setEndValue(1);
        ...
        for (int i = 1; i < mMenuItemCount; i++)
        {
            ...
            new Handler().postDelayed(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    springScaleX.addListener(new MapPerformer(menuItemView,
                            View.SCALE_X, 0, 1));
                    springScaleY.addListener(new MapPerformer(menuItemView,
                            View.SCALE_Y, 0, 1));
                    final DestroySelfSpringListener destroySelfSpringListener = new DestroySelfSpringListener(
                    ...
                    springX.addListener(new MapPerformer(menuItemView, View.X,
                            firstItem.getLeft(), menuItemView.getLeft()));
                    springY.addListener(new MapPerformer(menuItemView, View.Y,
                            firstItem.getTop(), menuItemView.getTop()));
                    springX.addListener(destroySelfSpringListener);
                    springY.addListener(destroySelfSpringListener);
                    springX.setEndValue(1);
                    springY.setEndValue(1);
            ...
            new Handler().postDelayed(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    springScaleY.addListener(new MapPerformer(menuItemView,
                            View.SCALE_Y, 1, 0));
                    final DestroySelfSpringListener destroySelfSpringListener = new DestroySelfSpringListener(
                            SpringFloatingActionMenu.this, mContainerView,
                            false);
                    springScaleX.addListener(destroySelfSpringListener);
                    springScaleY.addListener(destroySelfSpringListener);
                    springScaleX.setEndValue(1);
                    ...
                    springX.addListener(new MapPerformer(menuItemView, View.X,
                            menuItemView.getLeft(), firstItem.getLeft()));
                    springY.addListener(new MapPerformer(menuItemView, View.Y,
                            menuItemView.getTop(), firstItem.getTop()));
                    springX.addListener(destroySelfSpringListener);
                    springY.addListener(destroySelfSpringListener);
                    springX.setEndValue(1);
                    springY.setEndValue(1);
    ...
    public static class Builder
    {
        ...
        public Builder addMenuItem(@ColorRes int bgColor, int icon,
                String label, @ColorRes int textColor,
                View.OnClickListener onClickListener)
        {
            menuItems.add(new MenuItem(bgColor, icon, label, textColor,
                    onClickListener));
            return this;
        }
        
        public Builder addMenuItem(@ColorRes int bgColor, int icon,
                String label, @ColorRes int textColor, int diameter,
                View.OnClickListener onClickListener)
        {
            menuItems.add(new MenuItem(bgColor, icon, label, textColor,
                    diameter, onClickListener));
            return this;
        }
